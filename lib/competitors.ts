...
/** Hardcoded fallback — used when Google Search API is not configured or returns no results */
...
function hardcodedCompetitors(category: string): string[] {
...
/**
 * Returns competitor domains for the given category.
 * Tries Google Custom Search first (real ranking data);
 * falls back to hardcoded map if Search API is not configured or returns < 2 results.
 */
export async function getCompetitors(category: string): Promise<string[]> {
  // Try Google Search for real competitors
  const searchResults = await searchCompetitors(category);
  if (searchResults.length >= 2) {
    return searchResults;
  }

  // Fallback: hardcoded map
  return hardcodedCompetitors(category);
}
